...
for ((i = 0 ; i < N ; i++)); do
  PR=$((8000+i))
  ROUTER_PID=$(curl --no-progress-meter localhost:$PR/@/router/local | jq -r '.[0].value.pid')
  ...
  curl --no-progress-meter localhost:$PR/**  # Affiche les données insérées dans le router ayant pour clé /**
  echo ""
  echo ""
done
